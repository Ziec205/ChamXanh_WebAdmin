...
export class PlantsService {
  constructor(@InjectModel(Plant.name) private readonly model: Model<PlantDocument>) {}
  ...
  async taoMoi(du_lieu: Partial<Plant>) {
    if (await this.model.exists({ ma: du_lieu.ma })) {
      throw new ConflictException(`Mã "${du_lieu.ma}" đã tồn tại.`);
    }
    return this.model.create(du_lieu);
  }

  async capNhat(ma: string, du_lieu: Partial<Plant>) {
